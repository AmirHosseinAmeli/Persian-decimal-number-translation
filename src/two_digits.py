...
class TwoDigits(DecimalGroup):
    ...
    def __init__(self):
        ...
        self.cap_teen_n2pats = {10 + i: [DecimalGroup.var_subs.compile_vars('(@2dig.' + str(10 + i)) + ')'] for i in
                                range(10)}
        self.ncap_teen_n2pats = {10 + i: [DecimalGroup.var_subs.compile_vars('@2dig.' + str(10 + i))] for i in
                                 range(10)}
        ...
        self.cap_tenth_alphabet = DecimalGroup.var_subs.compile_vars('(@2dig.20' + ''.join(
            ['|@2dig.' + str(i) for i in range(30, 100, 10)]) + ')(?:\s*@and\s*(' + unity_pats + '))?')
        self.ncap_tenth_alphabet = DecimalGroup.var_subs.compile_vars('(?:@2dig.20' + ''.join(
            ['|@2dig.' + str(i) for i in range(30, 100, 10)]) + ')(?:\s*@and\s*(?:' + unity_pats + '))?')
        self.w2tenth = {DecimalGroup.var_subs.compile_vars('@2dig.' + str(i)): i for i in range(10, 100, 10)}
        self.cap_tenth_num = '([' + DecimalGroup.dig_pats + '])' + '([' + DecimalGroup.dig_pats + '])'
        self.ncap_tenth_num = '[' + DecimalGroup.dig_pats + ']{2}'

    def w2n(self, word):
        suc = re.search(self.aggregate_pats(self.cap_teen_n2pats), word)
        if suc:
            return self.teen_w2n[suc.group()]
        suc = re.search(self.cap_tenth_alphabet, word)
        if suc:
            res = self.w2tenth[suc.group(1)]
            if suc.group(2):
                res += self.unity.w2n(suc.group(2))
            return res
        suc = re.search(self.cap_tenth_num, word)
        if suc:
            return 10 * self.unity.w2n(suc.group(1)) + self.unity.w2n(suc.group(2))
        return self.unity.w2n(word)

    def related_pats(self):
        return '(?:'+self.aggregate_pats(self.ncap_teen_n2pats) + ')|(?:' + self.ncap_tenth_alphabet + ')|(?:' + \
               self.ncap_tenth_num + ')|(?:' + self.unity.related_pats() + ')'
